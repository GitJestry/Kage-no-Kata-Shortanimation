...
[[nodiscard]] bool hasSeverity(const TimelineValidation& parValidation,
  ...
  return std::any_of(parValidation.diagnostics.begin(),
                     parValidation.diagnostics.end(),
...
[[nodiscard]] bool testFrameRangesAndEvaluation() {
  ...
  const auto sequence = edits.createSequence(
      "Camera", {TimelineTargetKind::Camera, {7}}, base);
  ...
  if (!sequence || !edits.appendClipToLane(*sequence, 10, movement) ||
      !edits.appendClipToLane(*sequence, 10, fov) ||
      !edits.placeSequence(*sequence, 5) ||
...
[[nodiscard]] bool testMovementEvaluation() {
  ...
  if (!first_id || !second_id || !continuous ||
      ...
      !edits.setMovementStartMode(*second_id,
...
[[nodiscard]] bool testAnimationEvaluation() {
  ...
  constexpr std::array cases{
      ...
      LoopCase{"trimmed non-looping hold", false, 12, 10.0f / 30.0f * 0.5f,
               true},
  ...
  for (const LoopCase& item : cases) {
    ...
    TimelineEditService edits(timeline);
    const auto sequence = edits.createSequence(
        "Animation", {TimelineTargetKind::RiggedEntity, {14}},
    ...
    if (state.rig_animations.size() != 1 ||
        ...
        !close(state.rig_animations.front().local_time_seconds,
...
[[nodiscard]] bool testPropertyAndCameraGapEvaluation() {
  ...
  const auto first_camera = edits.createSequence(
      "First", {TimelineTargetKind::Camera, {21}}, camera_base);
  CapturedEntityBaseState second_camera_base = camera_base;
  second_camera_base.camera->vertical_fov_degrees = 70.0f;
  const auto second_camera = edits.createSequence(
      "Second", {TimelineTargetKind::Camera, {22}}, second_camera_base);
  CapturedEntityBaseState light_base;
  light_base.point_light = CapturedPointLightState{false, {0.2f, 0.4f, 0.8f},
                                                    2.0f, 42.0f, false};
  const auto light = edits.createSequence(
      "Light", {TimelineTargetKind::PointLight, {23}}, light_base);
  const auto sun = edits.createSequence("Sun", {TimelineTargetKind::Sun, {}},
  ...
  if (!first_camera || !second_camera || !light || !sun ||
      ...
      !edits.placeSequence(*light, 0) || !edits.placeSequence(*sun, 0)) {
    return false;
  }
...
[[nodiscard]] bool testTimelineEdits() {
  ...
  const std::array payload_cases{
      PayloadCase{"rig movement", TimelineTargetKind::RiggedEntity,
                  MovementClip{}, true},
      PayloadCase{"rig animation", TimelineTargetKind::RiggedEntity,
                  RigAnimationClip{.clip_id = 1}, true},
      ...
      PayloadCase{"camera animation", TimelineTargetKind::Camera,
                  RigAnimationClip{.clip_id = 1}, false},
  ...
  for (const PayloadCase& item : payload_cases) {
    ...
    TimelineEditService edits(timeline);
    const auto sequence = edits.createSequence(
        "Payload", {item.target, item.target == TimelineTargetKind::Sun
                                      ? scene::EntityId{}
                                      : scene::EntityId{41}},
        baseFor(item.target));
    const auto clip = sequence
                          ? edits.appendClipToLane(*sequence, 1, item.payload)
                          : std::expected<SequenceClipId, std::string>{
                                std::unexpected("setup failed")};
    if (!sequence || clip.has_value() != item.accepted ||
        timeline.sequences.front().clips.size() !=
            static_cast<std::size_t>(item.accepted)) {
  ...
  TimelineEditService edits(timeline);
  const auto sequence = edits.createSequence(
      "Actor", {TimelineTargetKind::RiggedEntity, {42}}, CapturedEntityBaseState{});
  const auto clip = sequence ? edits.appendClipToLane(*sequence, 10, MovementClip{})
                             : std::expected<SequenceClipId, std::string>{
                                   std::unexpected("setup failed")};
  const auto instance = clip ? edits.placeSequence(*sequence, 0)
  ...
  const auto camera_a = camera_edits.createSequence(
      "Camera A", {TimelineTargetKind::Camera, {51}}, camera_base);
  const auto camera_b = camera_edits.createSequence(
      "Camera B", {TimelineTargetKind::Camera, {52}}, camera_base);
  if (!camera_a || !camera_b ||
      !camera_edits.appendClipToLane(*camera_a, 10, MovementClip{}) ||
      !camera_edits.appendClipToLane(*camera_b, 10, MovementClip{}) ||
      !camera_edits.placeSequence(*camera_a, 0) ||
      !camera_edits.placeSequence(*camera_b, 5) ||
      !hasSeverity(validateMovieTimeline(camera_timeline),
  ...
  for (const DuplicateCase& item : duplicate_cases) {
    ...
    const auto source = full_lane_edits.createSequence(
        "Full lane", {item.target, {61}}, baseFor(item.target));
    const auto source_clip = source
                                 ? full_lane_edits.appendClipToLane(
                                       *source, MAX_FILM_FRAMES, item.payload)
                                 : std::expected<SequenceClipId, std::string>{
                                       std::unexpected("setup failed")};
    const SequenceClipId next_id = full_lane.next_clip_id;
    if (!source || !source_clip || full_lane_edits.duplicateClip(*source_clip) ||
        full_lane.sequences.front().clips.size() != 1 ||
        full_lane.next_clip_id != next_id) {
