...
[[nodiscard]] inline float getGizmoLength(const camera::Camera& parCamera,
                                          const glm::vec2& parViewportSize,
                                          const scene::EntityRecord& parEntity,
                                          const math::Bounds3& parBounds) {
  ...
  const float screen_length = camera::getWorldLengthForPixels(
      parCamera, parEntity.transform.transform.translation, parViewportSize, pixels);
  const glm::vec3 size = parBounds.getSize();
  const float mesh_length = parBounds.is_valid
                                ? std::max({size.x, size.y, size.z, 1.0f}) * GIZMO_MESH_EXTENT_SCALE
                                : 0.0f;
  return std::clamp(std::max(screen_length, mesh_length), GIZMO_MIN_LENGTH, GIZMO_MAX_LENGTH);
}
