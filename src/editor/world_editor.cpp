...
WorldEditor::WorldEditor(engine::EngineCore& parEngine)
    : m_engine(parEngine),
      m_paintbrush_controller(
          [this](std::size_t parAssetIndex, const kage::math::Transform& parTransform) {
            const scene::EntityId entity =
                m_engine.instantiateAssetAt(parAssetIndex, parTransform.translation);
  ...
  m_gizmo_controller.setAxisSpace(m_engine.getGizmoAxisSpace() == render::GizmoAxisSpace::World
...
void WorldEditor::update(float parDeltaSeconds, const input::EditorInputSnapshot& parInput) {
  ...
  m_engine.setGizmoAxisSpace(m_gizmo_controller.getAxisSpace() == GizmoController::AxisSpace::World
  ...
  if (parInput.key_z_pressed && !parInput.wants_capture_keyboard) {
    ...
    m_engine.setViewportMode(static_cast<render::ViewportMode>((current + 1) % 4));
  }
  if (m_session.workspace == Workspace::WorldEdit && parInput.key_delete_pressed &&
      !parInput.wants_capture_keyboard && !parInput.wants_capture_mouse) {
    if (!cancelActiveOperation()) {
      ...
      if (entity != nullptr) {
        ...
        request.message = "Delete \"" + entity->name.name + "\" (id " +
                          std::to_string(entity_id.value) + ")? This cannot be undone.";
        request.confirm_text = "Delete";
        request.cancel_text = "Cancel";
        request.destructive = true;
        request.on_confirm = [this, entity_id]() { m_engine.deleteEntity(entity_id); };
  ...
  const film::TargetSequence* selected_sequence =
      movie_workspace ? selectedMovieTargetSequence(m_session, m_engine.getMovieTimeline())
                      : nullptr;
  const bool camera_sequence_preview = playback.previewing && selected_sequence != nullptr &&
  ...
  const film::FilmFrame playback_duration =
      movie_workspace ? moviePreviewDuration(m_session, m_engine.getMovieTimeline()) : -1;
...
void WorldEditor::render(const glm::vec2& parViewportSize) {
  ...
  if (m_session.movie_selection.target.has_value() &&
      m_session.movie_selection.target->kind != film::TimelineTargetKind::Sun) {
  ...
  if (m_session.workspace == Workspace::Movie) {
    ...
    if (sequence != nullptr) {
      ...
      if (m_session.shown_movement_paths_sequence_id != sequence->id) {
        std::erase_if(movement_paths, [clip_id = selection.clip_id](const auto& movement) {
  ...
  m_engine.render(m_viewport, m_session.workspace == Workspace::Movie, -1.0, true, 0, 1,
                  preview_sequence_id, movie_selection_entity, movement_paths);
  m_engine.advanceFilmExport();
}

void WorldEditor::buildImGui(float parDeltaSeconds) {
  m_ui.draw(m_engine, m_session, m_placement_controller, m_selection_controller, m_gizmo_controller,
            m_confirmation_dialog, parDeltaSeconds);
...
void WorldEditor::registerDefaultAssets() {
  ...
  for (const std::filesystem::directory_entry& entry :
       std::filesystem::directory_iterator(paths.getModelDirectory(), error_code)) {
    if (error_code) {
      return;
    }
...
void WorldEditor::applyCameraMovement(const input::EditorInputSnapshot& parInput,
                                      bool parCameraSequencePreview) {
  const bool viewport_active = m_viewport.contains(parInput.framebuffer_cursor) &&
  ...
  const bool camera_keyboard_active =
      !parInput.wants_capture_keyboard && !parCameraSequencePreview &&
      (viewport_active || m_right_look_active || m_placement_controller.isActive() ||
       m_gizmo_controller.isActive());
...
void WorldEditor::handlePointerInput(const input::EditorInputSnapshot& parInput,
                                     bool parCameraSequencePreview) {
  ...
  const bool inside_viewport = m_viewport.contains(parInput.framebuffer_cursor);
  const glm::vec2 viewport_cursor = m_viewport.toLocal(parInput.framebuffer_cursor);
  const glm::vec2 viewport_size = m_viewport.extent();
  const bool cursor_over_panel = m_ui.isCursorOverPanel(parInput.ui_cursor);
  const bool ui_blocks_left = cursor_over_panel || parInput.wants_capture_mouse ||
  ...
  const bool right_look_start_area =
      inside_viewport && !cursor_over_panel && !parInput.ui_item_active && !parInput.ui_popup_open;
  if (!parInput.right_mouse_down) {
    m_right_look_active = false;
  }
  if (parInput.right_mouse_down && right_look_start_area && !m_right_look_active) {
  ...
  const glm::vec2 ui_viewport_size =
      glm::max(parInput.framebuffer_size / parInput.ui_to_framebuffer_scale, glm::vec2(1.0f));
  if (m_right_look_active && parInput.right_mouse_down) {
    m_engine.getCameraSystem().handleMouseMove(parInput.ui_delta, false, true, false,
                                               ui_viewport_size);
  }
  ...
  if (world_edit && m_ui.isPaintbrushEnabled() && viewport_active) {
    ...
    m_engine.setPaintbrushPreview(paintbrush_position,
  ...
  if (world_edit && m_placement_controller.isActive()) {
    if (viewport_active) {
      m_placement_controller.update(m_engine, viewport_cursor, viewport_size,
                                    parInput.left_mouse_down);
      if (parInput.left_mouse_pressed && m_placement_controller.canCommit()) {
  ...
  if (world_edit && m_gizmo_controller.isActive()) {
    m_gizmo_controller.update(m_engine, parInput.ui_delta * parInput.ui_to_framebuffer_scale,
                              viewport_cursor, viewport_size, parInput.left_mouse_down);
    publishGizmoGuide();
    return;
  }

  if (!viewport_active || !parInput.left_mouse_pressed || parInput.right_mouse_down) {
    return;
  }

  if (world_edit && m_gizmo_controller.begin(m_engine, viewport_cursor, viewport_size)) {
    publishGizmoGuide();
    return;
  }

  if (!world_edit) {
    static_cast<void>(m_selection_controller.selectMovieTarget(m_engine, m_session, viewport_cursor,
                                                               viewport_size));
    return;
  }

  const bool selected =
      m_selection_controller.handleViewportLeftPress(m_engine, viewport_cursor, viewport_size);
  if (!selected) {
    m_engine.clearSelection();
  }
}

void WorldEditor::updateViewportRect(const input::EditorInputSnapshot& parInput) {
  const bool movie_workspace = m_session.workspace == Workspace::Movie;
  const int width = std::max(static_cast<int>(parInput.framebuffer_size.x), 1);
  const int full_height = std::max(static_cast<int>(parInput.framebuffer_size.y), 1);
  if (movie_workspace) {
    ...
    const glm::ivec2 origin =
        glm::ivec2(glm::round(viewport.min * parInput.ui_to_framebuffer_scale));
    const glm::ivec2 size = glm::max(
        glm::ivec2(glm::round((viewport.max - viewport.min) * parInput.ui_to_framebuffer_scale)),
        glm::ivec2(1));
    m_viewport = {origin, size, full_height};
    return;
  }
  const int reserved_bottom = std::max(
      0, static_cast<int>(std::lround(UI_STATUS_HEIGHT * parInput.ui_to_framebuffer_scale.y)));
  m_viewport = {{0, 0}, {width, std::max(full_height - reserved_bottom, 1)}, full_height};
}

void WorldEditor::publishGizmoGuide() {
  const std::optional<render::GizmoGuide> guide = m_gizmo_controller.getActiveGuide(m_engine);
  if (guide.has_value()) {
