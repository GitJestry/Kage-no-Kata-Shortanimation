...
std::vector<PaintbrushScatterResult> PaintbrushScatterGenerator::generate(
  ...
  std::uniform_int_distribution<std::size_t> asset_distribution(0, parSelectedAssetIndices.size() - 1);
  ...
  for (int spawn_index = 0; spawn_index < spawn_count; ++spawn_index) {
    ...
    results.push_back({selected_asset_index,
                       kage::math::Transform{position, rotation, scale}});
  }
